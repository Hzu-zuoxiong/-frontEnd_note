...
* 如何判断当前脚本运行再浏览器还是Node环境中？

```
通过判断全局对象是否为window，如果不为window，当前脚本没有运行在浏览器中。在Node中的全局变量是global，浏览器 全局变量是window，
...
缺点：现在网站的JS都会进行压缩，一些文件用了严格模式，而另一些没有。这时本来是严格模式的文件，被Merge后，就串到文件中间，不仅
      没有指示严格模式，反而在压缩后浪费了字节。
```
...
也就是说GET需要通过地址栏来传值，而POST是通过提交表单来传值。
